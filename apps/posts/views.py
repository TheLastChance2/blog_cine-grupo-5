...
def modificar_posteo(request, id):
    ...
    posteos = get_object_or_404(Post, id = id)

    data = {
        'form': Fom_Post(instance=posteos)
    }

    if request.method == 'POST':
        formulario = Fom_Post(data=request.POST, instance=posteos, files=request.FILES)
        if formulario.is_valid():
            formulario.save()
            return redirect(to='/inicio/')
    return render(request, 'posts/MODIFICAR.html', data)
